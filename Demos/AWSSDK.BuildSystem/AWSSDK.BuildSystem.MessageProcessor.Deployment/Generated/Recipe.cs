...
using AWSSDK.BuildSystem.MessageProcessor.Deployment.Configurations;
...
namespace AWSSDK.BuildSystem.MessageProcessor.Deployment
{
    ...
    public class Recipe : Construct
    {
        ...
        private void ConfigureTaskDefinition(IRecipeProps<Configuration> props)
        {
            ...
            var otelContainerDefinition = new ContainerDefinitionOptions
            {
                Essential = false,
                ContainerName = "otel",
                ...
                {
                    StreamPrefix = props.StackName + "-OtelLogging"
                })
            };

            otelContainerDefinition.PortMappings = new IPortMapping[] { new PortMapping { ContainerPort = 4317, HostPort = 4317 } };

            var secrets = new Dictionary<string, Secret>();
            secrets["AOT_CONFIG_CONTENT"] = Secret.FromSsmParameter(StringParameter.FromStringParameterName(this, "AOTConfig", "otel-collector-config"));
            otelContainerDefinition.Secrets = secrets;

            var otelContainer = AppTaskDefinition.AddContainer(nameof(otelContainerDefinition), otelContainerDefinition);


            settings.ECSEnvironmentVariables["OTEL_RESOURCE_ATTRIBUTES"] = "AWSSDK Builder Processor";
            settings.ECSEnvironmentVariables["OTEL_EXPORTER_OTLP_ENDPOINT"] = "http://127.0.0.1:4317";
            ...
            var appContainer = AppTaskDefinition.AddContainer(nameof(AppContainerDefinition), InvokeCustomizeCDKPropsEvent(nameof(AppContainerDefinition), this, AppContainerDefinition));
            appContainer.AddContainerDependencies(new ContainerDependency() { Container = otelContainer, Condition = ContainerDependencyCondition.START });
        }
        ...
        private void ConfigureAutoScaling(Configuration settings)
        {
            ...
            if (settings.AutoScaling.Enabled)
            {
                ...
                {
                    case AWSSDK.BuildSystem.MessageProcessor.Deployment.Configurations.AutoScalingConfiguration.ScalingTypeEnum.Cpu:
                        ...
                        break;
                    case AWSSDK.BuildSystem.MessageProcessor.Deployment.Configurations.AutoScalingConfiguration.ScalingTypeEnum.Memory:
